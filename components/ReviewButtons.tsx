...
export default function ReviewButtons({id}:{id:string}){
 ...
 async function act(action:"approve"|"reject"){setBusy(action);try{await fetch(`/api/admin/properties/${id}/review`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action})});router.refresh();}finally{setBusy("");}}
